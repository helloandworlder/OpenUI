...
# Declare Variables
openui_folder="${OPENUI_INSTALL_DIR:=/usr/local/open-ui}"
openui_service="${OPENUI_SERVICE_DIR:=/etc/systemd/system}"
log_folder="${OPENUI_LOG_DIR:=/var/log/open-ui}"
mkdir -p "${log_folder}"
...
update() {
    confirm "This function will update all open-ui components to the latest version, and the data will not be lost. Do you want to continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/helloandworlder/OpenUI/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_menu() {
    ...
    curl -fLRo /usr/bin/open-ui https://raw.githubusercontent.com/helloandworlder/OpenUI/main/open-ui.sh
    chmod +x ${openui_folder}/open-ui.sh
    chmod +x /usr/bin/open-ui
...
legacy_version() {
    echo -n "Enter the OpenUI version (like 3.0.6):"
    read -r tag_version
    ...
    install_command="bash <(curl -Ls https://raw.githubusercontent.com/helloandworlder/OpenUI/main/install.sh) ${tag_version}"
...
uninstall() {
    ...
    if [[ $release == "alpine" ]]; then
        rc-service open-ui stop
        rc-update del open-ui
        rm /etc/init.d/open-ui -f
    else
        systemctl stop open-ui
        systemctl disable open-ui
        rm ${openui_service}/open-ui.service -f
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/helloandworlder/OpenUI/main/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        ${openui_folder}/open-ui setting -username "${config_account}" -password "${config_password}" -resetTwoFactor false > /dev/null 2>&1
    else
        ${openui_folder}/open-ui setting -username "${config_account}" -password "${config_password}" -resetTwoFactor true > /dev/null 2>&1
        echo -e "Two factor authentication has been disabled."
    fi

    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${green} Please use the new login username and password to access the OpenUI panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    ${openui_folder}/open-ui setting -webBasePath "${config_webBasePath}" > /dev/null 2>&1
...
reset_config() {
    ...
    ${openui_folder}/open-ui setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(${openui_folder}/open-ui setting -show true)
    if [[ $? != 0 ]]; then
    ...
    local existing_cert=$(${openui_folder}/open-ui setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
    local URL_lists=(
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        ${openui_folder}/open-ui setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        if [[ $release == "alpine" ]]; then
            rc-service open-ui start
        else
            systemctl start open-ui
        fi
        sleep 2
        check_status
        if [[ $? == 0 ]]; then
            LOGI "open-ui Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "open-ui and Xray stopped successfully"
        else
...
restart() {
    if [[ $release == "alpine" ]]; then
        rc-service open-ui restart
    else
        systemctl restart open-ui
    fi
    sleep 2
    check_status
    if [[ $? == 0 ]]; then
        LOGI "open-ui and Xray Restarted successfully"
    else
...
restart_xray() {
    if [[ $release == "alpine" ]]; then
        rc-service open-ui reload
    else
        systemctl reload open-ui
    fi
...
status() {
    if [[ $release == "alpine" ]]; then
        rc-service open-ui status
    else
        systemctl status open-ui -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    if [[ $release == "alpine" ]]; then
        rc-update add open-ui default
    else
        systemctl enable open-ui
    fi
    if [[ $? == 0 ]]; then
        LOGI "open-ui Set to boot automatically on startup successfully"
    else
        LOGE "open-ui Failed to set Autostart"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "open-ui Autostart Cancelled successfully"
    else
        LOGE "open-ui Failed to cancel autostart"
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                grep -F 'open-ui[' /var/log/messages
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                journalctl -u open-ui -e --no-pager -f -p debug
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
...
disable_bbr() {
    ...
    if [ -f "/etc/sysctl.d/99-bbr-open-ui.conf" ]; then
        old_settings=$(head -1 /etc/sysctl.d/99-bbr-open-ui.conf | tr -d '#')
        sysctl -w net.core.default_qdisc="${old_settings%:*}"
        sysctl -w net.ipv4.tcp_congestion_control="${old_settings#*:}"
        rm /etc/sysctl.d/99-bbr-open-ui.conf
        sysctl --system
    else
...
enable_bbr() {
    ...
    if [ -d "/etc/sysctl.d/" ]; then
        ...
        } > "/etc/sysctl.d/99-bbr-open-ui.conf"
        if [ -f "/etc/sysctl.conf" ]; then
...
update_shell() {
    curl -fLRo /usr/bin/open-ui -z /usr/bin/open-ui https://github.com/helloandworlder/OpenUI/raw/main/open-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ $release == "alpine" ]]; then
        if [[ ! -f /etc/init.d/open-ui ]]; then
            return 2
        fi
        if [[ $(rc-service open-ui status | grep -F 'status: started' -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        if [[ ! -f ${openui_service}/open-ui.service ]]; then
            return 2
        fi
        temp=$(systemctl status open-ui | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    if [[ $release == "alpine" ]]; then
        if [[ $(rc-update show | grep -F 'open-ui' | grep default -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        temp=$(systemctl is-enabled open-ui)
...
check_xray_status() {
    count=$(ps -ef | grep "${openui_folder}/bin/xray-" | grep -v "grep" | wc -l)
    if [[ count -ne 0 ]]; then
        return 0
    else
        return 1
...
update_geofiles() {
    ...
    for dat in "${dat_files[@]}"; do
        ...
        remote_file="${dat%%_*}"
        curl -fLRo ${openui_folder}/bin/${dat}.dat -z ${openui_folder}/bin/${dat}.dat \
            ...
            if [ -z "$domains" ]; then
                ...
                if echo "$domains" | grep -qw "$domain"; then
                    ...
                    if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                        ${openui_folder}/open-ui cert -webCert "$webCertFile" -webCertKey "$webKeyFile"
...
ssl_cert_issue_for_ip() {
    ...
    local existing_webBasePath=$(${openui_folder}/open-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(${openui_folder}/open-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    ...
    local reloadCmd="systemctl restart open-ui 2>/dev/null || rc-service open-ui restart 2>/dev/null"
...
ssl_cert_issue() {
    ...
    reloadCmd="open-ui restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}open-ui restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; open-ui restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                LOGI "Reloadcmd is: systemctl reload nginx ; open-ui restart"
                reloadCmd="systemctl reload nginx ; open-ui restart"
                ;;
            2)
                LOGD "It's recommended to put open-ui restart at the end, so it won't raise an error if other services fails"
                read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; open-ui restart): " reloadCmd
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(${openui_folder}/open-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(${openui_folder}/open-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(${openui_folder}/open-ui setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(${openui_folder}/open-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(${openui_folder}/open-ui setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    case "$num" in
        1)
            if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
                ...
                ${openui_folder}/open-ui setting -listenIP "${config_listenIP}" > /dev/null 2>&1
            ...
            ;;
        2)
            ${openui_folder}/open-ui setting -listenIP 0.0.0.0 > /dev/null 2>&1
...
show_usage() {
    echo -e "┌────────────────────────────────────────────────────────────────┐
│  ${blue}open-ui control menu usages (subcommands):${plain}                       │
│                                                                │
│  ${blue}open-ui${plain}                       - Admin Management Script          │
│  ${blue}open-ui start${plain}                 - Start                            │
│  ${blue}open-ui stop${plain}                  - Stop                             │
│  ${blue}open-ui restart${plain}               - Restart                          │
|  ${blue}open-ui restart-xray${plain}          - Restart Xray                     │
│  ${blue}open-ui status${plain}                - Current Status                   │
│  ${blue}open-ui settings${plain}              - Current Settings                 │
│  ${blue}open-ui enable${plain}                - Enable Autostart on OS Startup   │
│  ${blue}open-ui disable${plain}               - Disable Autostart on OS Startup  │
│  ${blue}open-ui log${plain}                   - Check logs                       │
│  ${blue}open-ui banlog${plain}                - Check Fail2ban ban logs          │
│  ${blue}open-ui update${plain}                - Update                           │
│  ${blue}open-ui update-all-geofiles${plain}   - Update all geo files             │
│  ${blue}open-ui legacy${plain}                - Legacy version                   │
│  ${blue}open-ui install${plain}               - Install                          │
│  ${blue}open-ui uninstall${plain}             - Uninstall                        │
└────────────────────────────────────────────────────────────────┘"
}
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│   ${green}OpenUI Panel Management Script${plain}               │
...
if [[ $# > 0 ]]; then
    case $1 in
        ...
        "update-all-geofiles")
            ...
            ;;
        "version")
            check_install 0 && ${openui_folder}/open-ui -v
            ;;
        *) show_usage ;;
    esac
else
